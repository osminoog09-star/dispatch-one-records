...
## По очереди и по силам (главный принцип работы)

Не сидим вдвоём на одной задаче/файле. Берём РАЗНЫЕ задачи по очереди, каждый — то,
что делает лучше.

Кто что умеет лучше:
- **Codex** — лаунчер, агент, сборка/релизы, бэкенд (`db.py`/`main.py`/импорт/`publish.py`),
  SQL/Supabase-логика, шлюз, данные и live-режим.
- **Claude** — визуал и UX сайта (шаблоны/`style.css`), тексты, пустые состояния,
  адаптивность, вид карты, UI-макеты новых страниц, QA видимых страниц.

Как берём:
1. У каждого агента в любой момент — максимум ОДНА активная задача (WIP-lock).
2. Закончил — берёшь следующую свободную задачу из очереди СВОЕЙ силы, не из чужой зоны.
3. Смешанная задача (данные + визуал) делится на подзадачи: сначала Codex делает
   бэкенд/данные и коммитит, потом Claude берёт визуал по готовому. Не одновременно
   в одном файле.
4. Если своя очередь пуста, а чужая нет — не лезь в чужую зону; напиши в `WIP.md`,
   что свободен, и подожди передачу от пользователя.
